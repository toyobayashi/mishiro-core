...
class Downloader {
  ...
  downloadAsset (hash, p, onData) {
    return this.downloadOne(`${Downloader.RES_HOST_BASE}/dl/resources/AssetBundles/${hash.substr(0, 2)}/${hash}`, p, onData)
      .then(p => {
  ...
  downloadDatabase (hash, p, onData, suffix = '.bdb') {
    return this.downloadOne(`${Downloader.RES_HOST_BASE}/dl/resources/Generic/${hash.substr(0, 2)}/${hash}`, p, onData)
      .then(p => {
        if (p && this.autoDecLz4) {
