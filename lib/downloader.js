...
class Downloader {
  constructor () {
    ...
    this.rename = true
    this.electronNet = false
  }

  useElectronNet (value) {
    this.electronNet = value
  }
  ...
  downloadOne (u, p, onData) {
    return new Promise((resolve, reject) => {
      ...
      const downloadStream = got.stream(u, {
        ...
        encoding: null,
        useElectronNet: !!this.electronNet
      })
  ...
  async batchDownload (manifests, targetDir, start, onData, complete, stop) {
    ...
    for (this.index = 0; this.index < this.tasks.length; this.index++) {
      ...
      const p = path.join(targetDir, path.basename(name, (ext !== '.acb' && ext !== '.awb') ? ext : ''))
