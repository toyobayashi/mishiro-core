...
exports.downloadManifest = (resVer, p, onData) => download(`http://storage.game.starlight-stage.jp/dl/${resVer}/manifests/Android_AHigh_SHigh`, p, onData).then(p => p && lz4dec(p, 'db'))

exports.downloadAsset = (hash, p, onData) => download(`${gameHostBase}/High/AssetBundles/Android/${hash}`, p, onData).then(p => p && lz4dec(p, 'unity3d'))

exports.downloadSound = (k, hash, p, onData) => download(`${gameHostBase}/High/Sound/Common/${k}/${hash}`, p, onData)

exports.downloadDatabase = (hash, p, onData, suffix = 'bdb') => download(`${gameHostBase}/Generic/${hash}`, p, onData).then(p => p && lz4dec(p, suffix))
