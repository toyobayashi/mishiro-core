...
class Downloader {
  ...
  downloadManifest (resVer, p, onData) {
    return this.downloadOne(`https://asset-starlight-stage.akamaized.net/dl/${resVer}/manifests/Android_AHigh_SHigh`, p, onData).then(p => p && lz4dec(p, '.db'))
  }
  ...
  downloadAsset (hash, p, onData) {
    ...
    return this.downloadOne(`${gameHostBase}/AssetBundles/${hash.substr(0, 2)}/${hash}`, p, onData).then(p => p && lz4dec(p, '.unity3d'))
  }
  ...
  downloadDatabase (hash, p, onData, suffix = '.bdb') {
    return this.downloadOne(`${gameHostBase}/Generic/${hash.substr(0, 2)}/${hash}`, p, onData).then(p => p && lz4dec(p, suffix))
  }
